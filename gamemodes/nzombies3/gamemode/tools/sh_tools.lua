...
if SERVER then
	util.AddNetworkString("nz_LegacyTools")
	
	net.Receive("nz_LegacyTools", function(len, ply)
		if nz.Rounds.Data.CurrentState == ROUND_CREATE and ply:Alive() then
			ply:Give( "nz_tool_zed_spawns" )
			ply:Give( "nz_tool_player_spawns" )
			ply:Give( "nz_tool_wall_buys" )
			ply:Give( "nz_tool_prop_modifier" )
			ply:Give( "nz_tool_door_locker" )
			ply:Give( "nz_tool_elec" )
			ply:Give( "nz_tool_block_spawns" )
			ply:Give( "nz_tool_random_box" )
			ply:Give( "nz_tool_random_box_handler" )
			ply:Give( "nz_tool_player_handler" )
			ply:Give( "nz_tool_perk_machine" )
			ply:Give( "nz_tool_barricades" )
			ply:Give( "nz_tool_ee" )
			ply:Give( "nz_tool_nav_locker" )
		end
	end)
end

nz.Tools.Functions.CreateTool("legacy", {
	displayname = "Legacy Tools",
	desc = "Use the button to give yourself legacy tools",
	condition = function(wep, ply)
		return true
	end,
	...
	interface = function(frame, data)
		...
		local text = vgui.Create("DLabel", panel)
		text:SetText("Click the button to get the old tools.")
		text:SetFont("Trebuchet18")
		text:SetTextColor( Color(50, 50, 50) )
		text:SizeToContents()
		text:SetPos(0, 90)
		...
		local text2 = vgui.Create("DLabel", panel)
		text2:SetText("These tools are outdated and no longer supported.")
		text2:SetFont("Trebuchet18")
		text2:SetTextColor( Color(150, 50, 50) )
		text2:SizeToContents()
		text2:SetPos(0, 100)
		...
		DermaButton:SetText( "Get Tools" )
		DermaButton:SetPos( 20, 120 )
		DermaButton:SetSize( 260, 30 )
		DermaButton.DoClick = function()
			net.Start("nz_LegacyTools")
			net.SendToServer()
		end
