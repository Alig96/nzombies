	...
	while (true) do
		...
		if (IsValid(target) and target:Alive()  ) then --and self:GetRangeTo(target) <= 1500
			...
			if (self:GetRangeTo(target) <= 42) then
				...
				if nz.Rounds.Curve.Speed[nz.Rounds.CurrentRound] >= 160 then
				...
				self.loco:SetDesiredSpeed(nz.Rounds.Curve.Speed[nz.Rounds.CurrentRound])
...
function ENT:OnInjured(damageInfo)
	...
	OnEnemyHurt( self, attacker, hitgroup )
end
